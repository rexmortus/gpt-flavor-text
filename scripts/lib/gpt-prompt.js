import * as utils from '/modules/gpt-flavor-text/scripts/lib/utils.js';

const promptInstructions = '. Provide a brief narration of this in the second-person for the player.';

export const createAttackRollGPTPrompt = (actor, item, target, scene, roll) => {
  ...
  let prompt = `${_actorName} attacks ${_targetName ? _targetName + ' ' : ''} using their ${_itemName} ${_sceneName ? 'in a/an ' + _sceneName : ', '} ${getHitMissPrompt(roll, target)} ${promptInstructions}`
...
function getHitMissPrompt(roll, target) {
    ...
    else if (value >= 0) { str = 'and lands a hit purely out of luck' }
