export const moduleName = "gpt-flavor-text";

export const gameSystems = (() => {
    const genericPrompt =
        "I would like you to help me with running the game by coming up with ideas, answering questions, and improvising. Keep responses as short as possible. Stick to the rules as much as possible.";
    const formatPrompt =
        "Always format each answer as HTML code without CSS, including lists and tables. Never use Markdown.";
    return {
        generic: {
...
export const registerSettings = () => {
    ...
    game.settings.register(moduleName, "gameSystem", {
        ...
        choices: Object.fromEntries(Object.entries(gameSystems).map(([id, desc]) => [id, desc.name])),
        onChange: (id) =>
            console.log(
                `${moduleName} | Game system changed to '${id}',`,
                "ChatGPT prompt now is:",
                getGamePromptSetting()
            ),
    });
    ...
    game.settings.register(moduleName, "rollAttack-autoPrompt", {
        ...
        hint:
            "Allow prompts to be automatically generated when enough info is available to do so." +
...
export const getGamePromptSetting = () => {
    return (
        game.settings.get(moduleName, "gamePrompt").trim() ||
